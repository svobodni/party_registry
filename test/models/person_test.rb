...
class PersonTest < ActiveSupport::TestCase
  ...
  should "handle paid for new supporter" do
    ...
    assert_transitions_from @person, :registered, to: :regular_supporter, on_event: :paid
  end

  should "handle paid for new member" do
    MemberNotifications.expects(:supporter_paid).returns(@mailer)
    @person = Person.new(status: :registered)
  ...
  should "handle paid for new member with fullfiled requirements" do
    ...
    @person.save(validate: false)
    @person.reload
    assert_transitions_from @person, :regular_supporter, to: :regular_member, on_event: :paid
  end

  should "handle paid supporter renewal payment" do
    @person = Person.new(status: :regular_supporter, paid_till: '2017-01-01')
    @person.save(validate: false)
    assert_transitions_from @person, :regular_supporter, to: :regular_supporter, on_event: :paid
  end
  ...
  should "handle paid member renewal payment" do
    ...
    @person.save(validate: false)
    assert_transitions_from @person, :regular_member, to: :regular_member, on_event: :paid
    #assert_emails 1
  end
